...
export function mengproductenPerSoort(products: Product[], limit = 6): MengproductKeuze[] {
  ...
  return [...perSoort.entries()]
    ...
    .map(([soort, lijst]) => ({
      soort,
      // Binnen een soort wint de échte mengverflijn: met foto en met meerdere
      // maten/basissen (dat onderscheidt een verflijn van een los potje), en
      // dan pas de laagste prijs.
      product: [...lijst].sort((a, b) => {
        const aScore = (a.image ? 2 : 0) + ((a.variants?.length ?? 0) > 1 ? 1 : 0);
        const bScore = (b.image ? 2 : 0) + ((b.variants?.length ?? 0) > 1 ? 1 : 0);
